...
class MyTestResult(result.TestResult):
    """A test result class that can print formatted text results to a stream.

    Used by MyTestRunner.
    """
    separator1 = '=' * 70
    separator2 = '-' * 70
    ...
    def addExpectedFailure(self, test, err):
        super(MyTestResult, self).addExpectedFailure(test, err)
        if self.showAll:
    ...
    def addUnexpectedSuccess(self, test):
        super(MyTestResult, self).addUnexpectedSuccess(test)
        if self.showAll:
